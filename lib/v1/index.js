...
P[ensureSymbol('spawnact')] = function(args) {
  ...
  if (typeof iter === 'function') {
    if (iter [get]('constructor.name', 0) === 'GeneratorFunction') iter = iter.apply(this, arguments);
    else return iter.apply(this, arguments);
  }

  if (iter [get]('constructor.constructor.name', 0) === 'GeneratorFunction') iter = iter [pro](this && this[status]);
  return iter || false;
};
...
P[ensureSymbol('by')] = function(gen, opts) {
  ...
  let getRes = (gen, opts) => function*(args) {
    ...
    } else if (iter) yield iter [spawnact].apply(iter, arguments);

    if (!gen) return;
    else if (typeof gen === 'function') {
      if (gen [get]('constructor.name', 0) === 'GeneratorFunction') return yield* gen.apply(this, arguments);
