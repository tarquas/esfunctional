...
P[ensureSymbol('spawnact')] = function(arg) {
  if (typeof this !== 'function') return this;
  if (this.constructor.name === 'GeneratorFunction') return spawn(this, arg);
  return this.apply(this, arguments) || false;
};
...
P[ensureSymbol('promisifyAll')] = unbind((object) => function(method) {
  if (!method) return object;
  if (arguments.length === 1) return object [promisify](method);
  let obj = object;
  for (let i = 0; i < arguments.length - 1; i++) obj = obj[arguments[i]];
  return object [promisify](arguments[arguments.length - 1]);
});
...
P[ensureSymbol('main')] = function(init, opts) {
  ...
  if (require.main === module) {
    ...
    if (!opts.stay) init [thena](exit);
    init [catcha](exceptionHandler);
    return true;
  }
