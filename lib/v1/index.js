 ...
 * @type Symbol
 */
global.promisify = global.unbind((object, method) => () => (
  new Promise((ok, nok) => (
    (typeof method === 'function' ? method : object[method])
    .apply(object, [].slice.call(arguments).concat(global.oknok(ok, nok)))
  ))
));
...
global.spawn = function(generatorFunc, arg) {
  ...
  if (generatorFunc.constructor.name !== 'GeneratorFunction') {
    return global.vala(arg) [global.thena] (generatorFunc);
  }
