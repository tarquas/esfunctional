...
if (global.esfunctional && version <= global.esfunctional) return;
...
P[ensureSymbol('spawnact')] = function(args) {
  ...
  if (typeof iter === 'function') {
    if (iter.constructor && iter.constructor.name === 'GeneratorFunction') iter = iter.apply(this, arguments);
    else return iter.apply(this, arguments);
  }

  if (iter.toString() === '[object Generator]') iter = iter [pro](this && this[status]);
  return iter || false;
};
...
P[ensureSymbol('by')] = function(gen, opts) {
  ...
  let getRes = (gen, opts) => function*(args) {
    if (gen && typeof gen !== 'function' && (!(gen instanceof Promise))) {opts = gen; gen = null;}
      ...
      if (opts.all) {
        ...
        let chunk = (
          ...
          .map((item, index) => (
            ...
            .then(res => ({index: index, cur: index, result: res})))
          )
        );
        ...
        for (; cur < iter.length; cur++) {
          ...
          chunk[got.index] = (
            item[spawnact]
            .apply(item, arguments)
            .then(res => ({index: got.index, cur: cur, result: res}))
          );
        }
    ...
    } else if (iter) yield iter;

    if (!gen) return;
    else if (typeof gen === 'function') {
      if (gen.constructor.name === 'GeneratorFunction') return yield* gen.apply(this, arguments);
      if (gen.toString() === '[object Generator]') return yield* gen;
      return gen.apply(this, arguments);
    } else if (gen instanceof Promise) {
      return yield gen;
    } else return mapping ? results [mapKeys]((v, k) => mapping[k].key) : results;
  };
...
global.promiseware = (func) => (req, res, next) => (
  ...
  [catcha]((err) => {
    try {
      if (err != null) err[errorware] = true;
    } catch (ee) {}
    next(err);
  })
);
