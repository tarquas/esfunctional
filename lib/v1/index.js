...
global.spawnAction = (action) => (
  typeof action !== 'function' ? action :
  action.constructor.name === 'GeneratorFunction' ? spawn(action) :
  action()
);
...
Object.prototype[global.all] = unbind((inputs, chunk) => spawn(function*(arg) {
  ...
  let chunkSize = chunk && (chunk.size | 0);
  ...
  if (chunkSize > 0) {
    ...
    for (let i = 0; i < actions.length; i += chunkSize) {
      ...
      let promises = slice.map(spawnAction);
      promises [forEach]((promise, j) => {statuses[mapping ? mapping[i + j].key : i + j] = promise[status];});
      let promise = Promise.all(promises);
      promise[halt] = (msg) => {slice.forEach(elem => {if (elem[halt]) elem[halt](msg);});};
      let chunkResult = yield promise [timeout](chunk.timeout, chunk.timeoutMsg);
      result.push.apply(result, chunkResult);
    }
  } else {
    let promises = actions.map(spawnAction);
    promises [forEach]((promise, j) => {statuses[mapping ? mapping[j].key : j] = promise[status];});
    let promise = Promise.all(promises);
    promise[halt] = (msg) => {promises.forEach(elem => {if (elem[halt]) elem[halt](msg);});};
    result = yield promise;
  }

  return mapping ? result [mapKeys]((v, k) => mapping[k].key) : result;
}));
...
Object.prototype[global.timeout] = function(msec, customMsg) {
  if (!msec) return this;

  let promise = Promise.race([this, new Promise((ok, nok) => (
    setTimeout(() => nok(new Error(customMsg || 'timeout')), msec)
  ))]);
...
Object.prototype[global.promisify] = global.unbind((object, method, opts) => function() {
  ...
  promise = new Promise((ok, nok) => {
    ...
    myHalt = (msg) => {
      ...
      nok(new Error(msg || 'halted'));
    };

    if (promise) promise[halt] = myHalt;

    func.apply(object, args.concat(global.oknok(ok, nok)));
  });
...
Object.prototype[global.catchify] = global.unbind((theny) => {
  if (theny[global.syncFail]) return theny;
...
global.halt = Symbol('halt');
global.status = Symbol('status');
global.current = Symbol('current');
...
global.spawn = function(generatorFunc, arg) {
  ...
  function continuer(verb, arg) {
    if (haltTree) return Promise.reject(new Error(haltTree));
    ...
    if (result) {
      if (result.done) {
        ...
        ret = Promise.resolve(result.value).then(onFulfilled, onRejected);
      }
    }
...
Object.prototype[global.event] = function(subs, throws) {
  ...
  let unsuball = () => unsubs.forEach(v => this.removeListener(v.sub, v.func));

  let p = Promise.defer();

  [subs || []] [flatten]().forEach(ev => subone(ev, (ev) => {unsuball(); p.resolve(ev);}));
  [throws || []] [flatten]().forEach(ev => subone(ev, (ev) => {unsuball(); p.reject(ev);}));

  p.promise[halt] = (msg) => {
    unsuball();
    p.reject(new Error(msg || 'halted'));
  };
...
global.test = Symbol('test');

Object.prototype[global.test] = function(what, that) {
