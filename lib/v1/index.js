...
Object.prototype[global.timeout] = function(msec, customMsg) {
  if (!msec) return this;
  let error = new Error(customMsg || 'timeout');
...
global.spawn = function(generatorFunc, arg) {
  ...
  function continuer(verb, arg) {
    if (haltTree) {
      error.message = new Error(haltTree);
...
Object.prototype[global.event] = function(subs, throws, opts) {
  if (!opts) opts = {};
  ...
  [throws || []] [flatten]().forEach(ev => subone(ev, (ev) => {unsuball(); p.reject(opts.detailError ? ev : ev.data);}));
