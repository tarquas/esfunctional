...
Object.prototype[global.all] = unbind((inputs, chunk) => spawn(function*() {
  let mapping = inputs instanceof Array ? null : inputs [map]((v, k) => ({key: k, value: v}));
  let promises = mapping ? mapping.map(item => item.value) : inputs;
  let result;
  ...
  if (chunkSize > 0) {
    ...
    for (let i = 0; i < promises.length; i += chunkSize) {
      ...
      let slice = promises.slice(i, i + chunkSize);
      let promise = Promise.all(slice);
