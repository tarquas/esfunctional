...
Object.prototype[global.event] = global.unbind((emitter, subs, throws) => global.spawn(function*() {
  var subsIndex = index(subs);
  var throwsIndex = index(throws);

  do {
    emitter.esfunctionalEventDispatcher = Promise.defer();
    let emitted = yield emitter.esfunctionalEventDispatcher.promise;
    let sub = emitted.event;
    if (!(sub in throwsIndex) && subs !== '*' && !(sub in subsIndex)) continue;
    emitter.esfunctionalEventDispatcher = null;
    var ret = {event: sub, data: emitted.data, args: emitted.args};
    if (!(sub in throwsIndex)) return ret;
 ...
 * @type Symbol
 */
global.subevents = global.eventstub = Symbol('eventstub');

Object.prototype[global.eventstub] = function() {
  let _this = this;
  let events = [].slice.call(arguments) [flatten]();

  events.forEach(event => this.on(event, function (data) {
    if (_this.esfunctionalEventDispatcher) _this.esfunctionalEventDispatcher.resolve({
      event: event,
      data: data,
